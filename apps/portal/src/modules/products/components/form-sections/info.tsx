...
export const ProductInfo = memo(function ProductInfo({ collections }: Props) {
  ...
  // Optimize form watching by using useMemo to prevent unnecessary re-renders
  const hasVariant = useMemo(() => form.watch("hasVariant"), [form]);

  // Memoize price error to avoid recalculating on every render
  const priceError = useMemo(() => form.getFieldState("price.selling").error, [form]);

  // Memoize the pricing section to prevent unnecessary re-renders
  ...
  }, [hasVariant, priceError]);

  // Memoize the classification section
  ...
  // Memoize the pricing mode section
  const pricingModeSection = useMemo(
    () => (
      <Card>
        <CardHeader>
          <CardTitle>
